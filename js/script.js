document.addEventListener("DOMContentLoaded", function () { 
    ...
    if (document.getElementById("cart-items")) {
        displayCart(); // Ladda varukorgen på cart.html
    }

    if (document.getElementById("checkout-items")) {
        displayCheckout(); // Ladda varukorgen på checkout.html
    }
});
...
function displayCheckout() {
    ...
    cart.forEach(item => {
        total += item.price * item.quantity;
        checkoutItemsDiv.innerHTML += `<p><strong>${item.name}</strong> - $${item.price} x ${item.quantity}</p>`;
    });

    document.getElementById("checkout-total").innerHTML = `<strong>Total: $${total.toFixed(2)}</strong>`;
}
